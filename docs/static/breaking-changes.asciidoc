...
* **Logging Output Directory Configuration:** The configuration parameter in `logstash.yml` for the log output directory has changed from `path.log` to `path.logs`. This is part of the switch to log4j as the default log output, which now produces a JSON-formatted log and a plain-text log by default. (See <<releasenotes>> for more).
